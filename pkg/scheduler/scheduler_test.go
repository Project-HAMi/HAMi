...
func Test_ResourceQuota(t *testing.T) {
	...
	tests := []struct {
			...
			quota: corev1.ResourceQuota{
				...
				Spec: corev1.ResourceQuotaSpec{
					Hard: corev1.ResourceList{
						"hami.io/gpucores": *resource.NewQuantity(200, resource.BinarySI),
		...
		},
		{
			...
			wantErr: fmt.Errorf("%d nodes %s(%s)", 1, parseNodeReason(common.ResourceQuotaNotFit)[0], strings.Join([]string{"node1"}, ",")),
			want: &extenderv1.ExtenderFilterResult{
				NodeNames: &[]string{},
			},
		},
		...
		},
		{
			...
			quota: corev1.ResourceQuota{
				...
				Spec: corev1.ResourceQuotaSpec{
					Hard: corev1.ResourceList{
						"hami.io/gpucores": *resource.NewQuantity(100, resource.BinarySI),
						"hami.io/gpumem":   *resource.NewQuantity(1500, resource.BinarySI),
					},
				},
			},
			wantErr: fmt.Errorf("%d nodes %s(%s)", 1, parseNodeReason(common.ResourceQuotaNotFit)[0], strings.Join([]string{"node1"}, ",")),
