...
func Test_MetaX_QoS_Quota_Validation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "Matching QoS policy (best-effort) fits on used card",
			qosPolicy: metax.BestEffort,
			coresReq:  30,
			devUsed:   1,
			devQos:    metax.BestEffort,
			wantFit:   true,
		},
		{
			name:      "Matching QoS policy (burst-share) fits on used card",
			qosPolicy: metax.BurstShare,
			coresReq:  30,
			devUsed:   1,
		...
		{
			name:       "Invalid or unknown QoS policy on used card rejected",
			qosPolicy:  "invalid-qos-policy",
			coresReq:   30,
			devUsed:    1,
			devQos:     metax.FixedShare,
			wantFit:    false,
