...
func (ns *NodeScore) OverrideScore(previous []*device.DeviceUsage, policy string) {
	...
	for idx, val := range ns.Devices {
		...
		// Backends implementing device.PolicyNeutralScorer return
		// policy-independent "higher is better" scores. For the Spread policy
		// (which selects the lowest score), we invert the score to preserve the
		// ranking. We also apply a weight of 10000 to ensure device scores
		// dominate the base node score. Keeping policy handling here means new
		// policies do not require backend-specific changes.
