...
// Against a real apiserver the delete above is graceful, so the stub being
// replaced is still there when Create runs. Swallowing AlreadyExists would
// announce a stub that still relays to the old endpoint as ready.
func TestReconcileSessionStubs_LeavesAStubStillTerminatingToTheNextPass(t *testing.T) {
	...
	fakeClient := sessionScene(t, nodes, stale)
	// The fake clientset deletes synchronously, so the graceful delete of a
	// real apiserver is what is reproduced here.
	fakeClient.PrependReactor("delete", "pods", func(k8stesting.Action) (bool, runtime.Object, error) {
		return true, nil, nil
	})
