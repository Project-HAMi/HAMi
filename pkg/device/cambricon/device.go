...
func (dev *CambriconDevices) ReleaseNodeLock(n *corev1.Node, p *corev1.Pod) error {
	...
	nodeToUpdate := current.DeepCopy()
	delete(nodeToUpdate.Annotations, DsmluLockTime)

	_, err = client.GetClient().CoreV1().Nodes().Update(ctx, nodeToUpdate, metav1.UpdateOptions{})
	for i := 0; i < retry && err != nil; i++ {
