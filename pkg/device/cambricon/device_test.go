...
func Test_ReleaseNodeLock(t *testing.T) {
	...
	t.Run("update conflict — retry succeeds", func(t *testing.T) {
		...
		cs.PrependReactor("update", "nodes", func(action k8stesting.Action) (bool, runtime.Object, error) {
			conflictCount++
			if conflictCount <= 2 {
				return true, nil, apierrors.NewConflict(schema.GroupResource{Resource: "nodes"}, "node-retry-01", fmt.Errorf("conflict"))
			}
		...
		if conflictCount < 2 {
			t.Errorf("expected at least 2 conflicts before success, got %d", conflictCount)
		}
	...
	t.Run("lock already removed during retry", func(t *testing.T) {
		...
		cs.PrependReactor("update", "nodes", func(action k8stesting.Action) (bool, runtime.Object, error) {
			return true, nil, apierrors.NewConflict(schema.GroupResource{Resource: "nodes"}, "node-retry-02", fmt.Errorf("conflict"))
		})
	...
	t.Run("re-get fails during retry", func(t *testing.T) {
		...
		cs.PrependReactor("update", "nodes", func(action k8stesting.Action) (bool, runtime.Object, error) {
			return true, nil, apierrors.NewConflict(schema.GroupResource{Resource: "nodes"}, "node-retry-03", fmt.Errorf("conflict"))
		})
		getCount := 0
		cs.PrependReactor("get", "nodes", func(action k8stesting.Action) (bool, runtime.Object, error) {
			getCount++
