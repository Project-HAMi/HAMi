...
func (sdev *MetaxSDevices) ScoreNode(node *corev1.Node, podDevices util.PodSingleDevice, previous []*util.DeviceUsage, pod *corev1.Pod) float32 {
	if !needScore(pod, podDevices) {
		return 0
	}
	...
	weight := 10000
	if userNodePolicy == string(util.NodeSchedulerPolicySpread) {
		weight = -10000
	}
...
func (mats *MetaxSDevices) Fit(devices []*util.DeviceUsage, request util.ContainerDeviceRequest, annos map[string]string, pod *corev1.Pod, allocated *util.PodDevices) (bool, map[string]util.ContainerDevices, string) {
	...
	for i := len(devices) - 1; i >= 0; i-- {
		...
		candidateDevices = append(candidateDevices, util.ContainerDevice{
			Idx:        int(dev.Index),
			UUID:       dev.ID,
			Type:       dev.Type,
			Usedmem:    memreq,
...
func needScore(pod *corev1.Pod, podDevices util.PodSingleDevice) bool {
	isTopoAware := MetaxTopologyAware
	if value, ok := pod.Annotations[MetaxSGPUTopologyAware]; ok {
