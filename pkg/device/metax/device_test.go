...
// declares its ScoreNode result as policy-independent so the shared scheduler
// policy layer applies the weight and Spread sign inversion.
func TestMetaxDevicesImplementsPolicyNeutralScorer(t *testing.T) {
	...
	if _, ok := dev.(device.PolicyNeutralScorer); !ok {
		t.Fatalf("MetaxDevices does not implement device.PolicyNeutralScorer")
	}
}
