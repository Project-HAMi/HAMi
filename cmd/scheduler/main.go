...
func init() {
	...
	rootCmd.Flags().IntVar(&config.NodeLockTimeout, "node-lock-timeout", 1, "timeout for node locks in minutes")
...
func start() error {
	...
	nodelock.NodeLockTimeout = time.Duration(config.NodeLockTimeout) * time.Minute
