...
* `nvidia.defaultCores`: 
  ...
  Note: When a container requests `nvidia.com/gpu` and its memory allocation is exclusive (for example `nvidia.com/gpumem-percentage` is 100, or neither memory percentage nor memory size is set and `nvidia.defaultMem` keeps the default 0), and the pod spec does not explicitly set `nvidia.com/gpucores`, HAMi defaults `nvidia.com/gpucores` to 100 during admission. Non-exclusive memory requests, or pods that already set `nvidia.com/gpucores`, remain unchanged.
