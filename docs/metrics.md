...
An example `scrape_config` for Prometheus that scrapes each node's monitor pod
directly:
...
If you scrape the device-plugin Service instead, note that its default type is
`NodePort` and it does not set `externalTrafficPolicy: Local`, so a request to
a node's address may be served by a monitor pod on a different node. To
guarantee per-node scraping, set `externalTrafficPolicy: Local` on the Service
or rely on the ServiceMonitor generated by the helm chart when
`prometheus.enabled` is true.
