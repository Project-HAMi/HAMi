...
These are instructions for an AI agent to analyze HAMi vGPU metrics and produce a structured usage summary for nodes, GPUs, pods, and namespaces.
...
#### 1.4 Cluster Mapping
Map each kubeconfig file to its cluster identity. File name format: `kubeconfig-gardenlogin--ait--<cluster-name>.yaml`.

| File Pattern | Cluster Alias | Environment |
|---|---|---|
| `*--dl-devops.yaml` | devops | 🧪 Test |
| `*--datalake-nonprod.yaml` | workspace | 🟡 Pre-prod |
| `*--dl-svc-provider.yaml` | svcProvider | 🔴 Prod |
| `*--ssdl-aigpu.yaml` | ssdl-aigpu | 🤖 AI GPU |

> Unrecognized files are flagged as `Unknown` and should still be checked.

#### 1.5 Authentication and Connectivity
...
#### 1.7 Ask the User to Choose the Kubeconfig for HAMi Metrics
